...
function loadData(type){const config=CONFIG[type];fetch(`./${config.dataFile}?v=${Date.now()}`).then(res=>{if(!res.ok)throw new Error('Data fetch failed');return res.json()}).then(data=>renderItems(data,config.igAccount)).catch(err=>{console.error('Error loading data:',err);renderEmpty()})}
