...
dataArr.forEach((member,index)=>{const isReverse=index%2!==0;const rowClass=isReverse?"reverse":"";const fotoFile=member.foto||"left-man.webp";let fotoPath=fotoFile;if(!fotoFile.includes("/")){if(fotoFile==="left-man.webp"||fotoFile==="right-man.webp"){fotoPath=`../global/assets/assets global/${fotoFile}`}else{const folderName=unitId.replace("sekbid","sekbid ");fotoPath=`../global/assets/assets struktur/${folderName}/${fotoFile}`}}
html+=`
...
window.addEventListener("popstate",(e)=>{const unit=e.state?.unit||getUnitFromURL();renderView(unit);setActiveStrip(unit)});function initHeroBg(){const bg=document.getElementById("sk-hero-bg");const img=new Image();img.onload=()=>bg.classList.add("loaded");img.src="../global/assets/assets global/Foto seluruh Osis.webp"}
